...
t0_Delta = Modified_Parameters_A_hat_.096["t0_Delta"]  #281.15
Temp = Temp_Imax+273.15
E_Delta = 0.5
Delta = Modified_Parameters_A_hat_.096["Delta"]
k = Modified_Parameters_A_hat_.096["k"]
...
ggarrange(ggarrange(per_capita_ingestion_graph, per_capita_metabolism_graph, per_capita_mortality_graph, per_capita_turnover_graph, ncol = 4, nrow = 1, labels = c("A", "B", "C", "D")),
 ...
 A_A_hat_Net_Production = Net_Production_A_hat_.096_A
  ...
  df3_R = df3[which(df3$name == "R"),]
  
  population_trajectory_graph = ggplot(df3_R, aes(x = Temperature, y= value)) + 
    theme_classic()+
    theme(text = element_text(size = 14),
 ...
 population_trajectory_graph = ggplot(df3, aes(x = Temperature, y= value)) + 
  ...
  df6_F = df6[which(df6$name == "birth_rate" & df6$Type == "Population"),]
  
  total_fecundity_graph = ggplot()+
    geom_line(data= df6_F, aes(x = Temperature, y = value))+
    labs(x= "Temperature (°C)", y="µg/day", title = "Birth Rate") +
    theme_classic()
